...
bool
http_post(const char *url, struct curl_slist *headers, const char *payload, HttpResponse *out_response)
{
    if (url == NULL || payload == NULL || out_response == NULL) {
        return false;
    }
...
void
http_free_response(HttpResponse *response)
{
